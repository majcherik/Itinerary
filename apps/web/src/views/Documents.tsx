...
const VisaSection: React.FC<VisaSectionProps> = ({ trip, updateTrip }) => {
...
const NotesSection: React.FC<NotesSectionProps> = ({ trip, addNote }) => {
                ...
                {notes.map(note => (
                    <div key={note.id} className="card">
                        ...
                        <ul className="text-sm text-text-secondary space-y-1">
                            {Array.isArray(note.content) ? (note.content as any[]).map((line, idx) => (
                                <li key={idx}>{line}</li>
                            )) : <li>{note.content}</li>}
                        </ul>
                    </div>
